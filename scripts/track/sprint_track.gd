...
## ruins rather than driving into a column.
func hazard_gate(offset: float) -> Vector2:
	for g in _gates:
...
func snap_to_track(global_pos: Vector3, advance: float = 0.0) -> Transform3D:
	...
	var gate: Vector2 = hazard_gate(off)
	if gate.y > 0.0:
...
## Two-wide grid behind the start line. Index 0 is front-left.
func get_grid_transform(index: int) -> Transform3D:
	@warning_ignore("integer_division")
	var row := index / 2
	var col := index % 2
	var off := maxf(start_offset - 12.0 - row * 9.5, 4.0)
	var f := frame_at(off)
...
func _make_obstacle(mesh: Mesh, shape: Shape3D, xform: Transform3D, color: Color,
	...
	var mi := MeshInstance3D.new()
	mi.mesh = mesh
	mi.material_override = ToonMaterial.make(color, 3.0, 0.5, 0.08)
	body.add_child(mi)
	add_child(body)
	...
	while o < to:
		for k in int(cfg["per_cluster"]):
			...
			if absf(lat - gate) < gate_w:
				continue
			var p: Vector3 = surface_point(oo, lat) + Vector3.UP * (height * 0.5 - 0.6)
			var tilt := Basis(Vector3.FORWARD, _rng.randf_range(-0.09, 0.09)) * \
					Basis(Vector3.UP, _rng.randf_range(0.0, TAU))
			_make_obstacle(mesh, shape, Transform3D(tilt, p), stone, "Pillar%d" % i)
			i += 1
		o += spacing
...
func _build_ice_patches(a: float, b: float, cfg: Dictionary) -> void:
	...
	for i in int(cfg["count"]):
		...
		var step := 2.0
		while o < end:
			var o2: float = minf(o + step, end)
			var pa := surface_point(o, centre - half_span) + Vector3.UP * 0.035
			var pb := surface_point(o, centre + half_span) + Vector3.UP * 0.035
			var pc := surface_point(o2, centre - half_span) + Vector3.UP * 0.035
			var pd := surface_point(o2, centre + half_span) + Vector3.UP * 0.035
			for v in [pa, pc, pd, pa, pd, pb]:
				st.set_normal(Vector3.UP)
				st.add_vertex(v)
			o = o2
		_make_body(st.commit(), ice.color, ice.id, "IcePatch%d" % i, 2.0, 0.62)
